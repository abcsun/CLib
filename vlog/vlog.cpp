...
Vlog::get_levels()
{
    ...
    for (size_t i=0; i < pimpl->n_modules() ; i++) {
		printf("Vlog::get_levels\n");
		printf("%-16s  %4s       %4s\n",pimpl->module_to_name[i].c_str(), get_level_name(pimpl->levels[FACILITY_CONSOLE][i]), get_level_name(pimpl->levels[FACILITY_SYSLOG][i]));
        /*string_printf(
            levels,
            "%-16s  %4s       %4s\n",
